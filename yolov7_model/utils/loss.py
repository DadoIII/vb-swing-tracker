...
#         return total_loss
    ...
    def distance_based_weights(self, elbow_labels, wrist_labels):
        """
        Calculates the weights based on the distance of elbow and wrist label pairs.

        Parameters:
            elbow_labels (torch.Tensor): Tensor holding the elbow labels with shape (1, batch_size)
            wrist_labels (torch.Tensor): Tensor holding the wrist labels with shape (1, batch_size)

        Returns:
            torch.Tensor: The weights for each of the scales of shape (num_of_scales, batch_size)
        """

        scales = torch.arange(1, self.num_of_scales)

        # Calculate the distance between elbow and wrist labels
        distances = torch.norm(elbow_labels - wrist_labels, dim=1).repeat(self.num_of_scales)
        # Define a function to map distances to weights
        # Adjust the function based on your specific requirements
        weights = torch.exp(-distance)  # Example: Inverse relationship between distance and weight
